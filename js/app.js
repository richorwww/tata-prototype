...
function injectCatIcons(lv){
  document.querySelectorAll('.poi[data-i]').forEach(function(el){
    ...
    if(lv>=1&&card){
      ...
      if((pl.isEvent||pl.hasEvent!==undefined)&&lv===1){
...
function mkCompactCard(pl,idx){
  return '<div class="cc2" data-poi-idx="'+idx+'" onclick="openDetail('+idx+')">'
    ...
    +(pl.hasEvent!==undefined?'<div class="rc-ev-badge"></div>':'')
...
function _setDetailTicket(pl){
  ...
  if(!sec)return;
  if(pl.fr){
    sec.innerHTML='';/* free tag shown in d-tags */
  } else if(pl.p){
    /* show 去购票 for any paid POI that has a URL */
    sec.innerHTML='<div class="d-ticket-block">'
      +'<div><div class="d-ticket-lbl">票价</div>'
      +'<div class="d-ticket-price">'+pl.p+'</div></div>'
      +(pl.url?'<a href="'+pl.url+'" target="_blank" class="d-buy-btn">去购票</a>':'')
      +'</div>';
  } else {
    sec.innerHTML='';
  }
...
function openDetail(idx){
  var pl=PL[idx];
  /* Redirect venue POIs with active events to show event detail first */
  if(pl&&pl.hasEvent!==undefined&&PL[pl.hasEvent]){openDetail(pl.hasEvent);return;}
  _dEvIdx=idx;
